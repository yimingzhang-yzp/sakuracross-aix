...
### 追加要件で置いた仮定(2026-09-08)

- **「金土日は週末営業」と「日月は定休日」の両立**: 日曜は両方に含まれるため、定休日を優先して既定では「休業」。曜日ルールは設定(`closedWeekdays` / `weekendWeekdays`)で変更できる。日曜を営業にする場合の提案種別は「週末営業」
- **法定控除の計算根拠**: 社会保険料は「標準報酬月額 × 料率 ÷ 2」(端数 50 銭以下切り捨て・超切り上げ)、雇用保険は「総支給 × 本人負担率」、源泉所得税は国税庁の税額表(月額表)ではなく **電算機計算の特例(甲欄)** の計算式を採用(電算機で計算する場合に認められた方法。税額表との差は数十円以内)。乙欄 88,000 円以上は自動計算できないため「源泉所得税の固定額」をスタッフに設定する運用
- **料率の既定値**: 健康保険 9.91%(協会けんぽ東京 令和 7 年度)、介護保険 1.59%、厚生年金 18.3%、雇用保険 本人負担 0.55%(一般の事業 令和 7 年度)。**都道府県・年度で変わるため設定画面で更新する**
- **令和 8 年分の所得税表**: 基礎控除の段階(95 万 / 88 万 / 68 万 / 63 万 / 58 万)を月額に換算した帯を `tax-tables.ts` に置いた。国税庁公表の「令和 8 年分 電算機計算の特例」と照合し、差異があれば同ファイルを更新する
- **標準報酬月額**: 本来は定時決定(4〜6 月の平均)で決まる値のため、スタッフごとに手入力する設計。未設定の場合のみ当月総支給から等級表で推定し、明細に警告を出す
- **介護保険(40〜64 歳)**: 生年月日を持たないため、スタッフの「介護保険の対象」チェックで管理する
- **社員の初期値**: マイグレーションで `employmentType = FULL_TIME` のスタッフを社保・雇用保険「加入」にした(アルバイトは未加入)。実態に合わせてスタッフ画面で修正する
- **住民税・賞与・年末調整**: 対象外(給与ソフト/社労士側)
